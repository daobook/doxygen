...
class Finder:
    ...
    def match(self,row):
      if self.row_type is int:
        ...
        return " REGEXP (?,%s)" %row if g_use_regexp == True else " %s=?" %row
...
def serveCgi():
  ...
  form = {key: fieldStorage.getvalue(key) for key in fieldStorage.keys()}
...
def serveCli(argv):
  ...
  for a, o in opts:
    ...
    cn=openDb(dbname)
    f=Finder(cn,o)
    j = processHref(cn,ref) if ref != None else process(f,kind)
